...
export function SpeechChallenge() {
  ...
  return (
    <div className="min-h-[100dvh] bg-background flex flex-col items-center justify-center p-4 md:p-8">
      {/* Decorative elements - keeping green/colorful for shapes */}
      <div className="fixed top-8 left-8 w-16 h-16 bg-chart-1 rounded-full border-4 border-foreground shadow-[4px_4px_0px_0px] shadow-foreground opacity-60" />
      <div className="fixed bottom-12 right-12 w-12 h-12 bg-chart-2 rounded-lg border-4 border-foreground shadow-[4px_4px_0px_0px] shadow-foreground rotate-12 opacity-60" />
      <div className="fixed top-1/4 right-8 w-8 h-8 bg-chart-3 rounded-full border-3 border-foreground opacity-50" />

      <div className="w-full max-w-2xl">
        {/* Header */}
        <header className={`text-center ${isIdle ? "mb-5 md:mb-6" : "mb-8 md:mb-12"}`}>
          <div className="inline-flex items-center gap-2 bg-secondary px-4 py-2 rounded-full border-3 border-foreground shadow-[4px_4px_0px_0px] shadow-foreground mb-4">
        ...
        <div className="bg-card border-4 border-foreground rounded-2xl shadow-[8px_8px_0px_0px] shadow-foreground overflow-hidden">
          ...
          {gameState === "countdown" && topic && (
            <div className="p-8 md:p-12 text-center">
              ...
              <div className="my-8">
                ...
                <div className="w-32 h-32 bg-primary rounded-full border-4 border-foreground shadow-[6px_6px_0px_0px] shadow-foreground mx-auto flex items-center justify-center animate-pulse">
                  <span className="text-6xl font-bold text-primary-foreground">{countdownTime}</span>
                </div>
              </div>
          ...
          {gameState === "playing" && topic && (
            <div className="p-6 md:p-8">
              ...
              {/* Topic */}
              <div className="bg-secondary p-6 rounded-xl border-4 border-foreground shadow-[4px_4px_0px_0px] shadow-foreground mb-6">
                <p className="text-sm font-bold uppercase tracking-wide text-muted-foreground mb-2">Your Topic</p>
                <h2 className="text-2xl md:text-3xl font-bold text-balance">{topic.question}</h2>
              </div>
                ...
                <div className="min-h-[100px] flex items-center justify-center">
                  {currentWordIndex >= 0 && currentWord ? (
                    <div className="animate-in zoom-in-50 fade-in duration-300">
                      <span className="inline-block bg-accent text-accent-foreground px-8 py-4 rounded-xl border-4 border-foreground shadow-[6px_6px_0px_0px] shadow-foreground text-3xl md:text-4xl font-bold">
                        {currentWord}
                      </span>
                    </div>
                  ) : (
